...
async def imagine(body: TriggerImagineIn = Body()):
...
async def upload_attachment(file: UploadFile):
  ...
  file_size = file.size
  attachment = await discord.upload_attachment(filename, file_size, await
                                               file.read())
...
async def queue_polling(body: QueueReleaseIn):
  ...
  if body.trigger_id not in RESULT_TABLE:
    raise HTTPException(status_code=404, detail="trigger_id not found")
  return RESULT_TABLE[body.trigger_id]
