...
from typing import Any, Callable, Deque, Dict, List, ParamSpec
...
class Task:
  ...
  def __init__(self, func: Callable[P, Any], *args: P.args,
...
class TaskQueue:
  ...
  def put(self, _trigger_id: str, func: Callable[P, Any], *args: P.args,
          **kwargs: P.kwargs) -> None:
    if len(self._wait_queue) >= self._wait_size:
      raise QueueFullError(f"Task queue is full: {self._wait_size}")

    self._wait_queue.append({_trigger_id: Task(func, *args, **kwargs)})
